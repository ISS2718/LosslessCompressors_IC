...
# Verifica se o script está sendo executado dentro do venv
PY_PATH=$(which python3)
if [[ "$PY_PATH" == *".venv"* ]]; then
    echo "✅ Usando Python do venv: $PY_PATH"
else
    echo "❌ Python não parece ser do venv ($PY_PATH)"
    exit 1
fi
...
compressors=("zlib" "gzip" "bz2" "ppmd" "webp_lossless" "jp2_lossless" "png" "entropy")
