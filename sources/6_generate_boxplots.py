...
dir = "../results/3_incremental_damicore/RESULTS"
images=f"../results/6_generate_boxplots"
...
for load in os.listdir(dir):
    ...
    for type in os.listdir(load_dir):
        ...
        df = df[(df['compressor'] != 'BZIP2') & (df['compressor'] != 'LZMA')]
        df["compressor"] = df["compressor"].astype(str).replace("heif", "hevc")

        df= remove_compressor_from_df(['BZIP2', 'LZMA','WEBP_LOSSY', 'JP2_LOSSY', 'HEVC'], df)
        df = remove_lossy_or_lossless_from_df(df)
        df = order_df_by_compressor(df, ['ZLIB', 'PPMD', 'GZIP','BZ2', 'PNG', 'JP2', 'WEBP', 'ENTROPY'])
